...
def main():
    ...
    def create_table():
        ...
        conn_db=mysql.connector.connect(host="localhost",user="greshan",password = "password")
        if conn_db.is_connected():
            cursor=conn_db.cursor()
            try:
                cursor.execute("CREATE DATABASE IF NOT EXISTS ASGN")
                cursor.execute("CREATE TABLE IF NOT EXISTS ASGN.rto_details(ID INT(10) PRIMARY KEY, RTOINCHARGE VARCHAR(255), Designation VARCHAR(255), PLACE varchar(255))")
                cursor.execute("CREATE TABLE IF NOT EXISTS ASGN.user_details(USERID INT(10), USERNAME varchar(255), VEHICLETYPE varchar(255), VEHICLENO varchar(255), VEHICLECOLOR varchar(255))")
...
@app.route('/RTO/list')
def display():
    conn_db=mysql.connector.connect(host="localhost",db="ASGN",user="greshan",password = "password")
    if conn_db.is_connected():
        cursor=conn_db.cursor()
        cursor.execute("SELECT * FROM ASGN.rto_details")
...
# read the posted values from the UI
    ...
    if conn_db.is_connected():
        cursor=conn_db.cursor()
        cursor.execute("SELECT * FROM ASGN.user_details")
